...
from imageprocessing import RetinalLesionDetector, segment_retinopathy_lesions
...
if __name__ == "__main__":
    ...
    if not image_files:
        print("Nu au fost găsite imagini!")  # "No images found!"
    else:
        for image_file in image_files:
            image_path = f"dataset/{image_file}"
            original, preprocessed_image, dark_lesions, bright_lesions, analysis = segment_retinopathy_lesions(image_path)
            filename = os.path.splitext(image_file)[0]

            # Acum includem parametrul analysis în apelul funcției
            generate_diagnosis(dark_lesions | bright_lesions, filename, analysis)
